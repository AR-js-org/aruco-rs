...
/// Configuration for a predefined ArUco dictionary.
#[derive(Debug, Clone)]
...
/// Predefined ArUco dictionaries matching `ARuco-ts`.
pub const DICTIONARY_ARUCO: DictionaryConfig = DictionaryConfig {
    n_bits: 25,
    tau: 3,
    code_list: &[
...
pub const DICTIONARY_ARUCO_MIP_36H12: DictionaryConfig = DictionaryConfig {
    n_bits: 36,
    tau: 12,
    code_list: &[
        0xd2b63a09d,
...
impl Dictionary {
    /// Initializes a new Dictionary from a configuration.
    pub fn new(config: &'static DictionaryConfig) -> Self {
        let mut codes = HashMap::with_capacity(config.code_list.len());
        for (idx, &code) in config.code_list.iter().enumerate() {
            codes.insert(code, idx);
        }

        let mark_size = (config.n_bits as f64).sqrt() as usize + 2;
    ...
    /// Converts a 2D bit array (or flat vector) into a single u64 integer representing the bitstring.
    /// Expected length of `bits` is `n_bits`.
    pub fn parse_code(&self, bits: &[u8]) -> u64 {
        let mut code: u64 = 0;
        for &bit in bits {
            code = (code << 1) | (bit as u64 & 1);
        }
        code
    }

    /// Calculates Hamming distance between two 64-bit integers comparing only `n_bits`.
    fn hamming_distance(&self, code1: u64, code2: u64) -> usize {
        // XOR the inputs, which gives 1s where they differ
        let xored = code1 ^ code2;
        // Construct a mask for the valid bits
        let mask = if self.n_bits == 64 {
            u64::MAX
        } else {
            (1 << self.n_bits) - 1
        };
        // Count the number of set bits within the mask
        (xored & mask).count_ones() as usize
    }

    /// Finds the closest match in the dictionary for the given bit sequence.
    pub fn find(&self, bits: &[u8]) -> Option<DictionaryMatch> {
        let code = self.parse_code(bits);
        let mut closest_match: Option<DictionaryMatch> = None;

        for (idx, &dict_code) in self.code_list.iter().enumerate() {
            let distance = self.hamming_distance(code, dict_code);
            if distance < self.tau {
                if let Some(mut current_match) = closest_match {
                    if distance < current_match.distance {
                        current_match.distance = distance;
                        current_match.id = idx;
                        closest_match = Some(current_match);
                    }
                } else {
                    closest_match = Some(DictionaryMatch { id: idx, distance });
                }
            }
        }
...
mod tests {
    ...
    fn test_dictionary_init() {
        ...
        assert_eq!(dict.n_bits, 25);
        assert_eq!(dict.tau, 3);
        assert_eq!(dict.mark_size, 7); // sqrt(25) + 2 = 5 + 2 = 7
        assert_eq!(dict.codes.len(), 1023);
    }
    ...
    fn test_parse_code() {
        ...
        bits[24] = 1; // 0000..0001 = 1
        assert_eq!(dict.parse_code(&bits), 1);

        // Test the first code in the dictionary
        // 0x1084210 = 0001000010000100001000010000 (29 bits?? wait, 0x1084210 is 0001 0000 1000 0100 0010 0001 0000 which is 25 bits)
        // 0x1084210 = 17318416
        // Let's create an array that parses to 0x1084210
        let mut first_code = vec![0u8; 25];
        let val = 0x1084210u64;
        for i in 0..25 {
            first_code[24 - i] = ((val >> i) & 1) as u8;
        }

        assert_eq!(dict.parse_code(&first_code), 0x1084210);
    }

    #[test]
    fn test_hamming_distance() {
        let dict = Dictionary::new(&DICTIONARY_ARUCO);
        assert_eq!(dict.hamming_distance(0b1101, 0b1001), 1);
        assert_eq!(dict.hamming_distance(0, 0), 0);
        assert_eq!(dict.hamming_distance(0x1084210, 0x1084217), 3); // 0...0 and 7...7 differ by 3 bits (111 vs 000)
    }
    ...
    fn test_find_exact_match() {
        ...
        let mut bits = vec![0u8; 25];
        for i in 0..25 {
            bits[24 - i] = ((val >> i) & 1) as u8;
        }

        let m = dict.find(&bits).unwrap();
        assert_eq!(m.id, 0);
        assert_eq!(m.distance, 0);
    }

    #[test]
    fn test_find_with_errors() {
        let dict = Dictionary::new(&DICTIONARY_ARUCO);
        // Take the first code and flip 1 bit
        let val = 0x1084210u64 ^ 1; // Flip the lowest bit
        let mut bits = vec![0u8; 25];
        for i in 0..25 {
            bits[24 - i] = ((val >> i) & 1) as u8;
        }
        ...
        let m = dict.find(&bits).unwrap();
        assert_eq!(m.id, 0); // Should still match id 0
        assert_eq!(m.distance, 1); // 1 bit error
    }

    #[test]
    fn test_find_no_match() {
        let dict = Dictionary::new(&DICTIONARY_ARUCO);
        // Let's use an array of alternating bits.
        // 0101010101010101010101010
        let val = 0x0AAAAAAu64;
        let mut bits = vec![0u8; 25];
        for i in 0..25 {
            bits[24 - i] = ((val >> i) & 1) as u8;
        }

        let m = dict.find(&bits);
        assert!(m.is_none());
    }
}
